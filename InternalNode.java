...
public class InternalNode extends Node {
	...
	public Node add(Metropole m, Point2D.Float origin, int width, int height) {
		final Point2D mLocation = m.remotetoPoint2D();
		for (int i = 0; i < 4; i++) {
			if (intersects(mLocation, regions[i])) {
				children[i] = children[i].add(m, origins[i], halfWidth,
						halfHeight);
				break;
			}
		}
	...
	public Node remove(Metropole m, Point2D.Float origin, int width,
		...
		for (int i = 0; i < 4; i++) {
			if (intersects(mLocation, regions[i])) {
				children[i] = children[i].remove(m, origins[i],
						halfWidth, halfHeight);
			}
		}
